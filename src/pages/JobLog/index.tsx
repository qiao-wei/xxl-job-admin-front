...
import { Button, message, FormInstance, Drawer, Row, Col } from 'antd';
...
import { queryLogs, clearLogs } from './service';
...
const JobLogList: React.FC = (props) => {
  ...
  const [showTriggerMsg, setShowTriggerMsg] = useState<boolean>(false);
  const [showHandleMsg, setShowHandleMsg] = useState<boolean>(false);

  const { location: { query: { jobId = 0 } } } = props;
    ...
    <PageContainer title={false}>
      ...
      >
        <Row>
          <Col span="10">
            <FormattedMessage
              id="pages.logs.executor-address"
              defaultMessage="执行器地址"
            />:
          </Col>
          <Col>
            {currentRow?.executorAddress}
          </Col>
        </Row>
        <Row>
          <Col span="10">
          ...
          <Col>
            {currentRow?.executorHandler}
          </Col>
        </Row>
        <Row>
          <Col span="10">
            <FormattedMessage
              id="pages.logs.executor-param"
              defaultMessage="任务参数"
            />:
          </Col>
          <Col>
            {currentRow?.executorParam}
          </Col>
        </Row>
      </Drawer>
      {clearModalVisible &&
        ...
        >
          <ProFormSelect
            ...
            request={async () => {
              return [
                { label: (<FormattedMessage id='pages.logs.clear.all' defaultMessage='全部' />), value: '9' },
                { label: (<FormattedMessage id='pages.logs.clear.month' defaultMessage='清理一个月之前的' />), value: '1' },
                { label: (<FormattedMessage id='pages.logs.clear.3month' defaultMessage='清理三个月之前的' />), value: '2' },
                { label: (<FormattedMessage id='pages.logs.clear.6month' defaultMessage='清理六个月之前的' />), value: '3' },
                { label: (<FormattedMessage id='pages.logs.clear.year' defaultMessage='清理一年之前的' />), value: '4' },
                { label: (<FormattedMessage id='pages.logs.clear.1000r' defaultMessage='清理一千条以前的' />), value: '5' },
                { label: (<FormattedMessage id='pages.logs.clear.10000r' defaultMessage='清理一万条之前的' />), value: '6' },
                { label: (<FormattedMessage id='pages.logs.clear.30000r' defaultMessage='清理三万条之前的' />), value: '7' },
                { label: (<FormattedMessage id='pages.logs.clear.100000r' defaultMessage='清理十万条之前的' />), value: '8' },
              ]
            }
            }
          />
      ...
      <Drawer
        width={460}
        visible={showTriggerMsg}
        onClose={() => {
          setCurrentRow(undefined);
          setShowTriggerMsg(false);
        }}
        closable={false}
      >
      ...
      <Drawer
        width={460}
        visible={showHandleMsg}
        onClose={() => {
          setCurrentRow(undefined);
          setShowHandleMsg(false);
        }}
        closable={false}
      >
